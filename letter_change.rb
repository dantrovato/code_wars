...
def letter_changes(str)
  ...
  str.chars.map do |char|
    case char
    when /[a-z]/ then alpha_down[(alpha_down.index(char) + 3) % 26]
    when /[A-Z]/ then alpha_up[(alpha_up.index(char) + 3) % 26]
    else              char
    end
  end.join
end
...
#       array[index] = chr.succ.squeeze if chr =~ /[A-Za-z]/
#     end
#   end
#   array.join
# end

p letter_changes('aB*jzEZ%')
p letter_changes('Axrj*Z&')
